...
> 🌙 **CatfishResearch keeps the ARIS workflow lineage and puts the Catfish Effect front and center.** The repo stays documentation-first, Codex-friendly, and conservative about code changes while the architecture evolves.
>
> 🪶 **Catfish Effect is the architectural concept.** It names the multi-project, parent-scored, provider-aware orchestration layer growing on top of the upstream ARIS workflow stack.
>
> *💡 ARIS remains the upstream methodology and provenance trail; CatfishResearch is the integrated repo identity for this worktree. 🌱*
...
CatfishResearch is the documentation-first integrated identity for this repository. It preserves the upstream ARIS mainline, the Codex-first fork history carried on `dev-intern-02`, and the current Catfish baseline: ARIS upstream merge tracking, Catfish architecture docs, runtime core, provider routing, control-center skeleton, `cc-switch` bridge, remote subagents, and the roadmap baseline for future evolution.

The workflow layer remains the proven ARIS/Codex stack for autonomous ML research. Claude Code or Codex can drive execution while an external LLM (via [Codex MCP](https://github.com/openai/codex)) acts as a critical reviewer. 🔀 **Also supports [alternative model combinations](#-alternative-model-combinations) (Kimi, LongCat, DeepSeek, etc.) — no Claude or OpenAI API required.** For example, [MiniMax-M2.7 + GLM-5 or GLM-5 + MiniMax-M2.7](docs/MiniMax-GLM-Configuration.md). 🤖 **[Codex CLI native](skills/skills-codex/)** — full skill set also available for OpenAI Codex. 🖱️ **[Cursor](docs/CURSOR_ADAPTATION.md)** — works in Cursor too. 🖥️ **[Trae](docs/TRAE_ARIS_RUNBOOK_EN.md)** — ByteDance AI IDE. 🚀 **[Antigravity](docs/ANTIGRAVITY_ADAPTATION.md)** — Google's agent-first IDE. 🆓 **[Free tier via ModelScope](docs/MODELSCOPE_GUIDE.md) — zero cost, zero lock-in.**

> **Provenance note for this branch:** CatfishResearch keeps the upstream ARIS mainline intact while preserving the Codex-first additions that were live on `dev-intern-02` at commit `405eaf5`. The local tree keeps both the upstream `skills/` hierarchy and the Codex-native `skills/skills-codex/` hierarchy, including `agentdoc-startup`, `pua-complex-task-method`, `remote-codex-subagents`, `heartbeat-subagent-template`, `final-summary-subagent`, `peer-review`, and the provider-route preview helper in `tools/codex_route_preview.py`.

## 🐟 Catfish Docs

Start here for the CatfishResearch architecture layer and current integrated baseline:

- [Catfish doc index](docs/catfish/INDEX_20260325.md)
- [ARIS merge record](docs/catfish/MERGE_ARIS_20260325.md)
- [System architecture](docs/catfish/ARCHITECTURE_20260325.md)
...
**Basic mode** — give CatfishResearch a research direction, it handles everything through the retained ARIS workflow layer:

```
/research-pipeline "factorized gap in discrete diffusion LMs"
```

**🔥 Targeted mode** — got a paper you want to improve? Give CatfishResearch the paper + the code:

```
/research-pipeline "improve method X" — ref paper: https://arxiv.org/abs/2406.04329, base repo: https://github.com/org/project
```

CatfishResearch uses the retained ARIS workflow stack to read the paper → find weaknesses → clone the codebase → generate ideas that specifically fix *those* weaknesses with *that* code → run experiments → write your paper. Like telling a research assistant: *"read this paper, use this repo, find what's missing, and fix it."*
...
**🔥 Rebuttal mode** — reviews just dropped? Don't panic. CatfishResearch reads every concern, builds a strategy, and drafts a rebuttal that's grounded, structured, and under the character limit:
...
### Codex-First Quick Start for This Fork
...
Recommended entrypoints in the Codex-native tree:
...
> **Branch-specific docs:** [`docs/CODEX_PUA_STACK.md`](docs/CODEX_PUA_STACK.md), [`docs/AGENTDOC_BRIDGE.md`](docs/AGENTDOC_BRIDGE.md), and [`docs/CODEX_CONTROL_PLANE.md`](docs/CODEX_CONTROL_PLANE.md) document the local Codex-first execution stack, AgentDoc bridge, and provider-route control-plane helpers carried by this fork.

> **Repository layout in this branch:** upstream ARIS workflows remain under `skills/`; the local Codex-first tree remains under `skills/skills-codex/`; review overlays remain under `skills/skills-codex-claude-review/` and `skills/skills-codex-gemini-review/`.
