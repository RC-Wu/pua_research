...
> **本分支补充文档：** [`docs/CODEX_PUA_STACK.md`](docs/CODEX_PUA_STACK.md)、[`docs/AGENTDOC_BRIDGE.md`](docs/AGENTDOC_BRIDGE.md)、[`docs/CODEX_CONTROL_PLANE.md`](docs/CODEX_CONTROL_PLANE.md) 说明了本地 Codex-first 执行栈、AgentDoc 桥接，以及 provider-route control-plane 辅助工具。

> **本分支目录约定：** upstream ARIS 工作流继续保留在 `skills/` 下；本地 Codex-first 技能树继续保留在 `skills/skills-codex/` 下；审稿 overlay 继续保留在 `skills/skills-codex-claude-review/` 与 `skills/skills-codex-gemini-review/` 下。
